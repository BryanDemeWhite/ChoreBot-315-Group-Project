...
namespace ChoreBot
{
    ...
    public partial class OldCMDs : ModuleBase<SocketCommandContext>
    {
        ...
        [Command("ping")]
        [Summary("A simple test for if the Bot is awake.")]
        public async Task pingAsync()
        {
            await Context.Channel.SendMessageAsync("pong :ping_pong:");
        }
        [Command("help")]
        [Summary("Outputs a list of commands and descriptions")]
        public async Task helpAsync(IUserMessage messageContext)
        {
        ...
        [Command("about")]
        [Summary("Outputs information about ChoreBot")]
        public async Task aboutAsync(IUserMessage m)
        {
            
            string a = "ChoreBot helps people coordinate their chores and tasks.\n" +
                    "Type **/Menu** into the message bar to open the menu, and then click the **Create Account** button to verify that you are ready to use the bot. " +
                    "Use the buttons in the menu to add chores, assign chores, complete, and review completed chores. " +
                    "When you complete a chore, you are rewarded points that can be used to purchase perks with the chore list. " +
                    "Check the Scoreboard to see who is the ChoreChampion of the server!\n" +
            ...
            if (lang != Translate.lang.English)
            {
                a = await Translate.TranslateText(a, lang);
            }

            await m.ReplyAsync(a, isTTS:tts);
        ...
        public async Task Gif([Remainder] string text)
        {
            ...
            await ReplyAsync(gif); // how to send message
